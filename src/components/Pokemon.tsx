...
*/
  ...
  useEffect(() => {
    ...
    axios.get(`https://pokeapi.co/api/v2/pokemon-species/${pokemonName}`).then((res) => {
      ...
      setEvochain(res.data.evolution_chain.url);
    });
    ...
    axios
      .get(evochain)
      .then((res) => {
        EvolChain.first.push(res.data.chain.species.name);
        EvolChain.second = res.data.chain.evolves_to.map((form: any) => form.species.name);
        if (EvolChain.second.length > 0) {
          for (let i = 0; i < EvolChain.second.length; i += 1) {
            EvolChain.third = res.data.chain.evolves_to[i].evolves_to.map(
